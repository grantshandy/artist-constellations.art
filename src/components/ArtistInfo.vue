<template>
	<div class="overflow-y-auto">
		<div class="rounded-md bg-base03 shadow-md py-2 space-y-2 text-center">
			<a v-bind:href="'https://open.spotify.com/artist/' + artist.id" target="_blank" rel="noopener noreferrer" class="block">
				<p class="md:text-2xl font-semibold mb-2">{{ artist.name }}</p>
				<img :src="artist.img" alt="Artist Profile Picture" class="rounded-md w-2/3 mx-auto">
			</a>
...
export default {
	...
	methods: {
		async revealTopTracks() {
			this.topTracks = await getArtistsTopTracks(this.artist)
				.catch(error => {
					this.error = error;
				});
